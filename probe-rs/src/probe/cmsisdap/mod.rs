...
/// Spec-safe fallback used when the probe firmware does not answer
/// `DAP_Info(Capabilities=0xF0)`. Every conforming CMSIS-DAP v1 probe
/// MUST support SWD, so setting `swd_implemented=true` and everything
/// else `false` gives probe-rs a minimum-viable capability set to build
/// on without over-claiming. Feature-detection code downstream sees
/// "SWD-only, no JTAG, no SWO trace" and picks the SWD-only code path.
/// FastLED/fbuild#930.
fn minimum_viable_capabilities() -> Capabilities {
    Capabilities {
        swd_implemented: true,
...
impl CmsisDap {
    fn new_from_device(mut device: CmsisDapDevice) -> Result<Self, DebugProbeError> {
        ...
        //
        // Older / incomplete CMSIS-DAP firmwares (LPC-Link2 v1.0.7 shipped on
        // NXP LPC845-BRK is the canonical example) do not respond to several
        // DAP_Info sub-commands at all — the probe just drops the request.
        // The old code would `?` propagate that as a `Timeout` error and
        // abort probe open, leaving the user with a hard-to-diagnose
        // "Failed to open the debug probe" message. The fix is to treat
        // timeouts on these individual queries as "field not implemented"
        // and fall back to CMSIS-DAP spec-safe defaults:
        //
        // - PacketCount default = 1: the spec's minimum. Single-packet
        //   sends are always correct; batching just isn't available.
        // - Capabilities default = SWD-only (see
        //   [`minimum_viable_capabilities`] below): every CMSIS-DAP v1
        //   probe MUST support SWD, so this is safe. JTAG / SWO / etc.
        //   stay disabled until we get a real Capabilities response.
        //
        // FastLED/fbuild#930 tracks the full sequence of upstream quirks
        // in this class.
        let packet_count = match commands::send_command(&mut device, &PacketCountCommand {}) {
            ...
            Err(CmsisDapError::Send {
                ...
                tracing::warn!(
                    "CMSIS-DAP probe did not respond to DAP_Info(PacketCount=0xFE); \
                     assuming spec-minimum single-packet transfer (see \
                     FastLED/fbuild#930)."
                );
                1
            }
            Err(e) => return Err(DebugProbeError::from(e)),
        };
        let caps: Capabilities = match commands::send_command(&mut device, &CapabilitiesCommand {})
        {
            Ok(c) => c,
            Err(CmsisDapError::Send {
                source: SendError::Timeout,
                ...
                tracing::warn!(
                    "CMSIS-DAP probe did not respond to DAP_Info(Capabilities=0xF0); \
                     falling back to SWD-only capability set (see FastLED/fbuild#930)."
                );
                minimum_viable_capabilities()
            }
            Err(e) => return Err(DebugProbeError::from(e)),
        };
        ...
        if caps.swo_uart_implemented || caps.swo_manchester_implemented {
            // If Capabilities said SWO exists, its size query is expected
            // to answer. Preserve the original semantics — no fallback.
    ...
    fn connect_if_needed(&mut self) -> Result<(), DebugProbeError> {
        ...
        // FastLED/fbuild#935: some very old CMSIS-DAP v1 firmware
        // (LPC-Link2 v1.0.7 shipped on NXP LPC845-BRK is the canonical
        // example) does not implement `DAP_Connect` with the `DefaultPort`
        // (0x00) selector — the firmware drops the HID report entirely
        // and probe-rs times out. Explicit `Swd` (0x01) is understood by
        // every conforming CMSIS-DAP v1 probe AND the buggy old
        // firmwares, so use it as the fallback when the caller did not
        // pin a specific protocol. We only try DefaultPort if the caller
        // explicitly requested it via a hypothetical future API — right
        // now everyone falls through to the "None → SWD" branch.
        //
        // Bonus: retry the Connect a small number of times to ride out
        // the "first HID report after enumeration gets dropped" behavior
        // some Windows HID stacks exhibit against this firmware.
        ...
        let mut last_err: Option<CmsisDapError> = None;
        let mut connect_result = None;
        for attempt in 0..4 {
            match commands::send_command(&mut self.device, &protocol) {
                Ok(v) => {
                    connect_result = Some(v);
                    break;
                }
                Err(e) => {
                    tracing::debug!(
                        "DAP_Connect attempt {}/4 failed: {}",
                        attempt + 1,
                        e
                    );
                    last_err = Some(e);
        ...
        let used_protocol = match connect_result {
            Some(v) => match v {
            ...
            None => {
                // All retries exhausted. Log and surface the last error.
                tracing::warn!(
                    "DAP_Connect (SWD) failed after 4 attempts against the CMSIS-DAP probe; \
                     the firmware may not implement `DAP_Connect` at all. See FastLED/fbuild#935."
                );
                return Err(DebugProbeError::from(
                    last_err.unwrap_or(CmsisDapError::ErrorResponse(RequestError::InitFailed {
                        protocol: self.protocol,
                    })),
                ));
            }
