...
impl CmsisDapDevice {
    ...
    ///
    /// If negotiation fails after all retries (older / buggy firmware that
    /// does not respond to `DAP_Info(PacketSize=0xFF)` at all — e.g. the
    /// LPC-Link2 CMSIS-DAP v1.0.7 shipped on NXP LPC845-BRK), fall back to
    /// the CMSIS-DAP v1 HID spec-mandated default of 64 bytes. The spec
    /// (CMSIS-DAP 2.1.0 §Interface Configuration → HID Interface) requires
    /// every implementation to accept 64-byte reports as the interoperable
    /// minimum, so this fallback is safe on any conforming probe.
    pub(super) fn find_packet_size(&mut self) -> Result<usize, CmsisDapError> {
        // CMSIS-DAP v1 HID spec-mandated default. Every conforming probe
        // MUST accept a 64-byte HID report; older firmware that doesn't
        // answer the size-negotiation query is safe to talk to at this rate.
        const CMSIS_DAP_V1_DEFAULT_HID_REPORT_SIZE: usize = 64;
        ...
        // Every attempt timed out. The probe firmware does not implement
        // DAP_Info(PacketSize=0xFF) at all — that's true of the LPC-Link2
        // CMSIS-DAP v1.0.7 that ships on NXP LPC845-BRK. Fall back to the
        // spec's 64-byte default rather than erroring out. Log at warn so
        // the fallback is visible under `RUST_LOG=warn`.
        tracing::warn!(
            "CMSIS-DAP probe did not respond to DAP_Info(PacketSize=0xFF) after 16 attempts; \
             assuming spec-default 64-byte HID report size (see CMSIS-DAP 2.1.0 § Interface \
             Configuration → HID Interface)."
        );
        self.set_usb_timeout(old_timeout);
        self.set_packet_size(CMSIS_DAP_V1_DEFAULT_HID_REPORT_SIZE);
        Ok(CMSIS_DAP_V1_DEFAULT_HID_REPORT_SIZE)
    }
