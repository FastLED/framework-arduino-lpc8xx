 ...
 *
 * LPC804 DMA controller. Same silicon block as LPC845's DMA0 (both are
 * documented as the NXP LPC8xx DMA controller in UM11065 §12 / UM11029
 * §17) — register offsets, COMMON register layout, and CHANNEL register
 * layout are identical. The difference is channel count: LPC804 ships
 * 4 channels vs LPC845's 25. Per UM11065 §12.4, LPC804 DMA channels 0..3
 * carry the following default trigger sources:
 *   - Channel 0: SPI0_TX
 *   - Channel 1: SPI0_RX
 *   - Channel 2: I2C0_MASTER
 *   - Channel 3: I2C1_MASTER
 * (Channels can also be routed via ITRIG_INMUX; the above are the
 * default direct triggers.)
 *
 * The block was intentionally omitted from the initial LPC804.h that
 * shipped with NXP's minimal LPC804 CMSIS PAL — this declaration adds
 * it back so downstream drivers (FastLED #3499 / #3453 Phase 1 SPI DMA)
 * can consume the standard `DMA0` symbol on both chips.
 */
typedef struct {
